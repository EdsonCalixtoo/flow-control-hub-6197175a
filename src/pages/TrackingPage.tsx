...
import { fetchOrderById } from '@/lib/orderServiceSupabase';
...
const TrackingPage: React.FC = () => {
    ...
    useEffect(() => {
        loadOrder();
        // Polling para "tempo real" sem precisar de ERPContext autenticado
    ...
    if (!order) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-slate-500 max-w-xs mt-2">Verifique se o link está correto ou entre em contato com seu vendedor.</p>
                <Link to="/" className="mt-8 text-primary font-bold hover:underline transition-all">Voltar ao início</Link>
            </div>
        );
    }

    // Pipeline steps for the customer
    const steps = [
        { key: 'rascunho', label: 'Orçamento Criado', icon: ShoppingBag, color: 'text-slate-400' },
        {
        ...
        <div className="min-h-screen bg-slate-50 pb-12">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-sm font-black text-slate-800 leading-tight">FLOW CONTROL</h1>
            ...
            <div className="max-w-xl mx-auto p-4 sm:p-6 space-y-6">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
                    ...
                    <div className="relative space-y-8 ml-3">
                        {/* Connecting Line */}
                        <div className="absolute left-4 top-2 bottom-2 w-0.5 bg-slate-100 -z-1" />
                        ...
                        {steps.map((step, idx) => {
                            ...
                            return (
                                <div key={idx} className={`flex gap-6 relative animate-in fade-in slide-in-from-left duration-500 delay-${idx * 100}`}>
                                    <div className={`
                                        w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-500
                                        ${isCompleted ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 scale-100' :
