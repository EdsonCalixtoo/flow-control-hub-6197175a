...
const AprovacoesPage: React.FC = () => {
  const { orders, clients, products, updateOrderStatus, loadFromSupabase, loadOrderDetails } = useERP();
  const { user } = useAuth();
  ...
  const pendentes = orders.filter(o => {
    if (o.status !== 'aguardando_financeiro') return false;
  ...
  const rejeitar = async (orderId: string) => {
    try {
      await updateOrderStatus(orderId, 'rejeitado_financeiro', { rejectionReason: rejectReason }, userName, `Rejeitado: ${rejectReason}`);
      setSelectedOrderId(null); 
      setShowReject(false); 
      setRejectReason('');
    } catch (error) {
      ...
      {selectedOrder ? (
        <div className="card-section p-6 space-y-5 animate-scale-in">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { setSelectedOrderId(null); setShowReject(false); }} className="btn-modern bg-muted text-foreground shadow-none text-xs px-3 py-1.5">
            ...
            <div className="space-y-3 stagger-children">
              {pendentes.map(order => (
                <div key={order.id} className="card-section p-5 flex items-center justify-between flex-wrap gap-3 hover:shadow-lg hover:shadow-primary/[0.04] transition-all duration-300">
                  ...
                  <button onClick={() => setSelectedOrderId(order.id)} className="btn-modern bg-primary/10 text-primary shadow-none text-xs px-4 py-2 hover:bg-primary/20">
