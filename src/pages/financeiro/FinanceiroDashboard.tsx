        ...
        <div className="space-y-6">
          ...
          {activeTab === 'pedidos' && (
            <div className="space-y-6 animate-fade-in">
              <div className="card-section p-6 space-y-6 glass-premium">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="relative flex-1 min-w-[300px] group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground/40 group-focus-within:text-primary transition-colors duration-300" />
                    <input
                      type="text"
                      placeholder="Buscar por número do pedido ou nome do cliente..."
                      value={searchQuery}
                      onChange={e => { setSearchQuery(e.target.value); setCurrentPage(1); }}
                      className="w-full pl-12 pr-4 py-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-border/50 hover:border-primary/40 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-300 rounded-2xl text-sm font-semibold shadow-[0_2px_10px_rgb(0,0,0,0.02)] placeholder:text-muted-foreground/50"
                    />
                  </div>
                  <button
                    onClick={() => setShowFilters(f => !f)}
                    className={`h-[56px] px-8 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all duration-500 flex items-center justify-center gap-2 ${showFilters ? 'bg-primary text-white shadow-[0_8px_30px_rgba(14,165,233,0.3)] hover:shadow-[0_8px_30px_rgba(14,165,233,0.4)] hover:-translate-y-0.5' : 'bg-white/60 dark:bg-slate-900/60 text-muted-foreground border border-border/50 hover:bg-white dark:hover:bg-slate-800 hover:text-foreground hover:shadow-lg hover:-translate-y-0.5'}`}
                  >
                    <Filter className="w-4 h-4" /> Filtros Avançados
                    <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-500 ${showFilters ? 'rotate-180' : ''}`} />
                  </button>
                </div>
              ...
              <div className="card-section bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/20 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] rounded-[2rem] overflow-hidden">
                <div className="p-6 border-b border-border/10 bg-gradient-to-r from-slate-50/50 to-transparent dark:from-slate-800/50 flex items-center justify-between relative overflow-hidden">
                  <div className="absolute inset-0 bg-grid-slate-200/[0.04] bg-[bottom_1px_center] dark:bg-grid-slate-800/[0.04] dark:bg-[bottom_1px_center]" style={{ maskImage: 'linear-gradient(to bottom, transparent, black)' }} />
                  <h3 className="text-xs font-black text-foreground uppercase tracking-[0.25em] flex items-center gap-3 relative z-10">
                  ...
                  <span className="relative z-10 text-[10px] font-black text-primary px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 shadow-sm">
                ...
                <div className="overflow-x-auto">
                  <table className="modern-table">
                    <thead>
                      <tr className="bg-slate-50/50 dark:bg-slate-800/30 text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">
                        <th className="cursor-pointer select-none py-5 px-6 hover:text-primary transition-colors" onClick={() => handleSort('number')}>
                          <div className="flex items-center gap-2">PEDIDO {sortBy === 'number' && (sortDir === 'asc' ? '↑' : '↓')}</div>
                        </th>
                        <th className="cursor-pointer select-none text-left py-5 px-6 hover:text-primary transition-colors" onClick={() => handleSort('clientName')}>
                          <div className="flex items-center gap-2">CLIENTE {sortBy === 'clientName' && (sortDir === 'asc' ? '↑' : '↓')}</div>
                        </th>
                        <th className="hidden md:table-cell text-left py-5 px-6">LOGÍSTICA / VENDEDOR</th>
                        <th className="cursor-pointer select-none text-right py-5 px-6 hover:text-primary transition-colors" onClick={() => handleSort('total')}>
                          <div className="flex items-center justify-end gap-2">VALOR TOTAL {sortBy === 'total' && (sortDir === 'asc' ? '↑' : '↓')}</div>
                        </th>
                        <th className="py-5 px-6">ESTÁGIO</th>
                        <th className="text-right py-5 px-6 pr-8">AÇÕES</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/10">
                      {paginatedOrders.filter(o => !isOrderCarenagem(o)).map(order => {
                        ...
                        return (
                          <tr key={order.id} className={`group hover:bg-white dark:hover:bg-slate-800/80 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.3)] transition-all duration-500 relative z-0 hover:z-10 ${order.isSite ? 'bg-blue-50/20 shadow-lg shadow-blue-500/5' : ''}`}>
                            <td className="font-black text-foreground py-6 px-6 select-text">
                              <div className="flex items-center gap-3">
                            ...
                            <td className="text-foreground py-6 px-6">
                              <div className="flex flex-col">
                            ...
                            <td className="hidden md:table-cell py-6 px-6">
                            ...
                            <td className="text-right font-black text-foreground py-6 px-6 tabular-nums tracking-tighter text-base">{formatCurrency(order.total)}</td>
                            <td className="py-6 px-6"><StatusBadge status={order.status} /></td>
                            <td className="text-right py-6 px-6 pr-8">
                              <div className="flex items-center justify-end gap-3">
                                <button
                                  onClick={() => setSelectedOrderId(order.id)}
                                  className="h-10 w-10 rounded-xl bg-slate-100/80 dark:bg-slate-800/80 border border-border/40 text-primary shadow-sm hover:shadow-md hover:scale-110 active:scale-95 hover:bg-white dark:hover:bg-slate-700 transition-all duration-300 flex items-center justify-center"
                                  title="Ver Detalhes"
                                >
                                  <Eye className="w-4 h-4" />
                                </button>
