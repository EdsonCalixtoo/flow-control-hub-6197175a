...
import { DollarSign, TrendingUp, Clock, AlertTriangle, Search, Filter, ChevronDown, ChevronLeft, ChevronRight, Eye, CheckCircle, XCircle, Send, ArrowLeft, Users2, BarChart3, Radio, Star, Plus, Trash2, Inbox, Bell, FileText, Package, Info, BadgeCheck, MapPin, Box, CreditCard } from 'lucide-react';
            ...
            <div className="card-premium p-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { label: 'Cliente Final', value: selectedOrder.clientName, icon: Users2, detail: client?.cpfCnpj ? `CPF/CNPJ: ${client.cpfCnpj}` : 'Sem documento' },
                    { label: 'Consultor de Vendas', value: selectedOrder.sellerName, icon: BadgeCheck, detail: 'Responsável direto' },
                    { label: 'Logística / Frete', value: selectedOrder.carrier || 'Logística Própria', icon: Send, detail: selectedOrder.orderType === 'retirada' ? 'Retirada no Local' : 'Entrega Padrão' },
                    { label: 'Fluxo Operacional', value: isOrderCarenagem(selectedOrder) ? 'Direto p/ Estoque' : 'Fluxo de Produção', icon: Radio, detail: 'Processamento automático' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center gap-4 p-5 rounded-3xl bg-slate-50 dark:bg-slate-900/40 border border-border/40 group hover:border-primary/30 transition-all hover:shadow-lg hover:shadow-primary/[0.02]">
                      ...
                      <div className="min-w-0">
                        <span className="text-[10px] uppercase font-black text-muted-foreground/60 block tracking-widest">{item.label}</span>
                        <span className="text-sm font-black text-foreground block truncate">{item.value}</span>
                        <span className="text-[10px] text-muted-foreground block mt-0.5">{item.detail}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {selectedOrder.paymentStatus !== 'pago' && saldoDevedor <= 0 && (
                   <button
                    onClick={async () => {
                      ...
                      setSelectedOrder({ ...selectedOrder, status: newStatus as any, paymentStatus: 'pago', statusPagamento: 'pago', financeiroAprovado: true });
            ...
            {selectedOrder.status === 'aguardando_financeiro' && (
              <div className="space-y-4">
                {!showReject ? (
                  ...
                  <div className="card-premium p-6 bg-rose-500/5 border-rose-500/20 space-y-4 animate-scale-in">
                    ...
                    <textarea
                      ...
                      placeholder="Motivo do indeferimento financeiro..."
                      className="input-modern min-h-[120px] bg-white dark:bg-slate-900 border-rose-500/20 py-4 text-xs font-semibold focus:border-rose-500/50"
                    />
                    <div className="flex gap-2">
                       <button
                        onClick={() => rejeitarPedido(selectedOrder.id)}
                        disabled={!rejectReason.trim()}
                        className="flex-1 py-3 rounded-xl bg-rose-500 text-white font-black text-[10px] uppercase tracking-widest disabled:opacity-50"
                      >
                        Confirmar
                      </button>
                      <button
                        onClick={() => { setShowReject(false); setRejectReason(''); }}
                        className="px-4 py-3 rounded-xl bg-muted text-muted-foreground font-black text-[10px] uppercase tracking-widest"
                      >
                        Voltar
                      </button>
                    </div>
