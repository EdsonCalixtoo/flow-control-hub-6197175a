...
import { 
  CheckCircle, Clock, AlertTriangle, Search, ExternalLink, CreditCard, Landmark, QrCode, Banknote, XCircle 
} from 'lucide-react';
...
const PagamentosPage: React.FC = () => {
  const { orders, products, updateOrderStatus, addFinancialEntry, loadFromSupabase } = useERP();
  
  // 📡 Sincronização em Tempo Real para esta página
  useRealtimeOrders((event) => {
    loadFromSupabase?.();
    if (event.type === 'UPDATE' && event.order.status === 'aguardando_financeiro') {
      toast.info(`Pedido #${event.order.number} atualizado com novo comprovante.`);
    }
  });
                ...
                <tr key={order.id} className="group hover:bg-primary/[0.02] transition-colors">
                  ...
                  <td className="py-6">
                    {order.receiptUrl ? (
                      <button 
                        type="button"
                        onClick={() => setPreviewUrl(cleanR2Url(order.receiptUrl || ''))}
        ...
        >
          ...
          <div className="flex-1 overflow-auto flex items-center justify-center p-8" onClick={e => e.stopPropagation()}>
            {previewUrl.startsWith('data:application/pdf') || previewUrl.toLowerCase().includes('.pdf') ? (
              ...
              <img src={cleanR2Url(previewUrl)} alt="Comprovante" className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl" />
            )}
          </div>
        </div>
      )}
