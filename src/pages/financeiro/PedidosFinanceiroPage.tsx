        ...
        return (
            <div className="space-y-6 animate-scale-in">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Galeria de Comprovantes (Agregado) */}
                    {(() => {
                        ...
                        // 1. Capturar comprovante principal do pedido
                        ...
                        // 2. Capturar anexos adicionais do pedido
                        ...
                        // 3. Capturar comprovantes dos lançamentos financeiros (pagamentos parciais)
                        ...
                        return (
                            <div className="lg:col-span-3 p-6 rounded-3xl border border-primary/20 bg-primary/5 shadow-sm relative z-20 pointer-events-auto">
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground mt-1 font-medium font-bold">Total de {allVouchers.length} arquivo(s) encontrado(s)</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    {allVouchers.map((rec, idx) => (
                                        <a
                                            key={idx}
                                            href={cleanR2Url(rec.url)}
                                            target="_blank"
                                            rel="noreferrer"
                                            className="flex items-center gap-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-border/40 hover:border-primary/40 transition-all text-left shadow-sm group cursor-pointer pointer-events-auto relative z-30"
                                        >
                                            <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                                                {rec.url.toLowerCase().includes('.pdf') ? <FileText className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[10px] font-black text-foreground uppercase truncate tracking-tight">{rec.title}</p>
                                                <p className="text-[9px] text-muted-foreground font-bold uppercase truncate flex items-center gap-1">
                                                     <span className={`px-1 rounded ${rec.category === 'Venda' ? 'bg-indigo-500/10 text-indigo-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                                                        {rec.category}
                                                    </span>
                            ...
                            <div className="space-y-3">
                                ...
                                {(selectedOrder.status === 'aprovado_financeiro' || selectedOrder.status === 'aguardando_producao') && (
                                    <button
                                        onClick={async () => {
                                            try {
                                                await updateOrderStatus(selectedOrder.id, 'em_producao', undefined, 'Financeiro', 'Liberado para produção');
