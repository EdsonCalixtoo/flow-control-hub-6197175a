            ...
            <div className="cl-form-card" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                <div style={{ display: 'flex', gap: '20px' }}>
                  <div style={{ width: '64px', height: '64px', borderRadius: '18px', background: 'linear-gradient(135deg,#8b5cf6,#6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: '800', color: '#fff' }}>
                    {selectedClient.name?.charAt(0)?.toUpperCase()}
                  </div>
                  <div>
                    <h2 className="cl-text-title" style={{ fontSize: '22px', fontWeight: '800', margin: 0 }}>{selectedClient.name}</h2>
                    <p className="cl-text-subtitle" style={{ fontSize: '13px', marginTop: '4px' }}>{selectedClient.cpfCnpj}</p>
                    {selectedClient.consignado && (
                      <div className="cl-badge" style={{ background: 'rgba(251,191,36,0.15)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.3)', marginTop: '8px' }}>
                        <Icon.Star /> Cliente Consignado
                      </div>
                    )}
                  </div>
                </div>
                <button className="cl-btn-ghost" onClick={() => setSelectedClient(null)} style={{ padding: '8px' }}>
                  <Icon.X />
                </button>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px', marginBottom: '32px', flexWrap: 'wrap' }}>
                ...
                >
                  <Icon.Cart /> Criar Orçamento
                </button>
                {selectedClient.phone && (
                  <button
                    className="cl-btn-success"
                    style={{ background: '#25D366', flex: 1, minWidth: '160px' }}
                    onClick={() => window.open(`https://wa.me/55${selectedClient.phone.replace(/\D/g, '')}`, '_blank')}
                  >
                    <Icon.WhatsApp /> WhatsApp
                  </button>
                )}
                <button className="cl-btn-ghost" onClick={() => openEdit(selectedClient)} style={{ flex: 1, minWidth: '120px' }}>
                  <Icon.Edit /> Editar Dados
                </button>
              </div>

              {/* Info grid */}
              <div className="cl-grid-2" style={{ marginBottom: '32px' }}>
                <div className="cl-section">
                  <div className="cl-section-title">Contato</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ fontSize: '13px', margin: '0 0 8px' }}>
                      <span className="cl-text-subtitle" style={{ fontSize: '11px', display: 'block', textTransform: 'uppercase', marginBottom: '2px' }}>Telefone</span>
                      <span className="cl-text-title">{selectedClient.phone || '—'}</span>
                    </div>
                    <div style={{ fontSize: '13px', margin: 0 }}>
                      <span className="cl-text-subtitle" style={{ fontSize: '11px', display: 'block', textTransform: 'uppercase', marginBottom: '2px' }}>E-mail</span>
                      <span className="cl-text-title">{selectedClient.email || '—'}</span>
                    </div>
                  </div>
                </div>
                <div className="cl-section">
                  <div className="cl-section-title">Localização</div>
                  <p className="cl-text-title" style={{ fontSize: '13px', margin: 0, lineHeight: 1.5 }}>
                    <span className="cl-text-subtitle" style={{ fontSize: '11px', display: 'block', textTransform: 'uppercase', marginBottom: '4px' }}>Endereço Completo</span>
                    {selectedClient.address}<br />
                    {selectedClient.bairro && `${selectedClient.bairro} — `}{selectedClient.city}/{selectedClient.state}<br />
                    {selectedClient.cep}
                  </p>
                </div>
              </div>
              ...
              <div>
                <div className="cl-section-title" style={{ color: 'rgba(255,255,255,0.6)' }}>
                  <Icon.History /> Histórico de Pedidos
                </div>
                <div style={{ marginTop: '12px' }}>
                  {orders.filter(o => o.clientId === selectedClient.id).length === 0 ? (
                    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '12px', padding: '32px', textAlign: 'center' }}>
                      <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', margin: 0 }}>Nenhum pedido realizado para este cliente ainda.</p>
                    </div>
                  ) : (
                          ...
                          >
                            <div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span className="cl-text-title" style={{ fontSize: '14px', fontWeight: '800' }}>#{order.number}</span>
                                <StatusBadge status={order.status} />
                              </div>
                              <p className="cl-text-subtitle" style={{ fontSize: '11px', margin: '4px 0 0' }}>
                                {new Date(order.createdAt).toLocaleDateString('pt-BR')} • Total: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(order.total)}
                              </p>
                            </div>
                            <button
                              className="cl-btn-primary"
              ...
              {selectedClient.notes && (
                <div style={{ marginTop: '32px', padding: '16px', borderRadius: '12px', background: 'rgba(251,191,36,0.05)', border: '1px solid rgba(251,191,36,0.1)' }}>
                  <span style={{ fontSize: '11px', fontWeight: '800', color: '#fbbf24', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: '6px' }}>Observações Internas</span>
                  <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', margin: 0, fontStyle: 'italic' }}>{selectedClient.notes}</p>
                </div>
              )}
